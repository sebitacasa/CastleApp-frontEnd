...
const SELECTABLE_CATEGORIES = ['Castles', 'Museums', 'Historic Site', 'Ruins', 'Religious', 'Others'] as const;
...
const SearchScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const [creatingCoords, setCreatingCoords] = useState<CreatingCoords | null>(null);
  const [loadingSubmit, setLoadingSubmit] = useState<boolean>(false);
  const [uploadingImage, setUploadingImage] = useState<boolean>(false);

  const [formData, setFormData] = useState<FormData>({
    name: '', description: '', imageUri: null, category: 'Castles',
  });

  const loadPlaces = async (): Promise<void> => {
    const data = await getLocations();
    if (data && Array.isArray(data)) setPlaces(data as LocationData[]);
  };
      ...
      if (creatingCoords) {
        try {
          ...
          if (reverseGeo.length > 0) {
            ...
            addressText = city ? `${city}, ${country}` : (country || 'Unknown');
          }
        } catch { /* silent */ }
      }
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContainer}>
          ...
          >
            ...
            <Text style={styles.modalSubtitle}>
              Help map the forgotten history. Add details for the location at the center of the map.
            </Text>
            ...
            <View style={{ height: 50, marginBottom: 20 }}>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                {SELECTABLE_CATEGORIES.map((cat) => (
                  ...
                  >
                    <Text style={[styles.catOptionText, formData.category === cat && styles.catOptionTextSelected]}>
                      {cat}
                    </Text>
                  </TouchableOpacity>
                ))}
...
const styles = StyleSheet.create({
  ...
  createText: { fontWeight: 'bold', fontSize: 15, marginLeft: 8, color: THEME.bg, letterSpacing: 1 },
  ...
  closeModalBtn: { padding: 5, zIndex: 10 },
  imagePickerBtn: {
