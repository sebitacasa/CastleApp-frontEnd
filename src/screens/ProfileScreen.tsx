...
import {
  View, Text, Image, TouchableOpacity, StyleSheet, StatusBar, ScrollView,
  ImageBackground, Platform, Alert, Modal, TextInput, KeyboardAvoidingView,
  ActivityIndicator,
} from 'react-native';
...
const MenuOption: React.FC<MenuOptionProps> = ({ icon, label, onPress, color, noBorder }) => (
...
const ProfileScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [conquestRank, setConquestRank] = useState<ConquestRank | null>(null);
  ...
  useEffect(() => {
    if (!userToken) return;
    axios
      .get(`${API}/api/conquests/rank`, { headers: { Authorization: `Bearer ${userToken}` } })
      .then(r => setConquestRank(r.data))
      .catch(() => {});
    axios
      .get(`${API}/api/username`, { headers: { Authorization: `Bearer ${userToken}` } })
  ...
  const saveUsername = async (): Promise<void> => {
    ...
    try {
      ...
      const msg = e.response?.data?.error === 'taken'
  ...
  const userName: string =
    ...
    (userInfo as any)?.user_metadata?.full_name ||
    'Explorer';
    ...
    Alert.alert(
      ...
      [
        { text: t('profile.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const userId = (userInfo as any)?.id || (userInfo as any)?.user?.id;
              if (!userId) {
                Alert.alert('Error', t('profile.deleteIdError'));
                return;
              }
              await axios.delete(
                `https://castleapp-backend-production.up.railway.app/auth/${userId}`,
              );
              logout();
            } catch {
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileHeader}>
          ...
          <Text style={styles.locationText}>
            <MaterialCommunityIcons name="compass-outline" size={14} color={THEME.gold} />
            {' '}{t('profile.historySeeker')}
          </Text>
        </View>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 14 }}>
              ...
              <Text style={styles.rankSub}>
                ...
                {conquestRank.rank?.next
                  ? ` · ${(conquestRank.rank.nextCount ?? 0) - conquestRank.total} to ${conquestRank.rank.next}`
                  : ' · Max rank! 👑'}
              </Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color={THEME.subText} />
        ...
        <View style={styles.menuContainer}>
          <MenuOption icon="people-outline" label={t('profile.friends')} color={THEME.text} onPress={() => navigation.navigate('Friends', {})} />
          <MenuOption icon="map-outline" label={t('profile.myDiscoveries')} color={THEME.text} onPress={() => navigation.navigate('MyDiscoveries')} />
          <MenuOption icon="shield-outline" label={t('profile.myConquests')} color={THEME.text} onPress={() => navigation.navigate('Conquests', {})} />
          <MenuOption icon="heart-outline" label={t('profile.myFavorites')} color={THEME.text} onPress={() => navigation.navigate('Favorites')} />
          <MenuOption icon="notifications-outline" label={t('profile.notifications')} color={THEME.text} onPress={() => {}} />
          <MenuOption icon="log-out-outline" label={t('profile.logOut')} color={THEME.text} onPress={logout} />
          <MenuOption icon="trash-bin-outline" label={t('profile.deleteAccount')} color={THEME.danger} onPress={handleDeleteAccount} noBorder />
        </View>
...
const styles = StyleSheet.create({
  ...
  headerNav: {
    ...
    paddingTop: Platform.OS === 'ios' ? 60 : 50, paddingHorizontal: 20, marginBottom: 20,
  },
  headerTitle: {
  ...
  avatar: { width: 100, height: 100, borderRadius: 50, borderWidth: 3, borderColor: THEME.gold },
  rankBadge: {
  ...
  locationText: { color: THEME.subText, fontSize: 14 },
  usernameRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4, marginTop: 2 },
  usernameText: { fontSize: 13, color: THEME.gold, fontWeight: '600' },
  ...
  uploadOverlay: {
    flex: 1, backgroundColor: 'rgba(244, 241, 234, 0.7)',
  ...
  uploadSubtitle: {
    fontSize: 13, color: THEME.text, textAlign: 'center', marginTop: 5, maxWidth: '90%', lineHeight: 18,
  },
  menuContainer: {
    backgroundColor: THEME.card, marginHorizontal: 20, borderRadius: 16,
    padding: 10, borderWidth: 1, borderColor: THEME.border, elevation: 2,
  ...
  menuBorder: { borderBottomWidth: 1, borderBottomColor: THEME.border },
  ...
  modalSaveBtn: { backgroundColor: THEME.gold, paddingVertical: 10, paddingHorizontal: 24, borderRadius: 20 },
  modalSaveText: { color: THEME.bg, fontWeight: 'bold', fontSize: 15 },
  rankCard: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: THEME.card, marginHorizontal: 20, marginBottom: 20,
    borderRadius: 16, borderWidth: 1, borderColor: THEME.border, padding: 16,
  },
