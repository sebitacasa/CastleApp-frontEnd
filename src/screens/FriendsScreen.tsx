...
function getRankEmoji(count: number): string {
  let r = RANKS[0];
...
function Avatar({ uri, name, size = 44 }: AvatarProps): React.ReactElement {
...
export default function FriendsScreen({ navigation, route }: Props): React.ReactElement {
  const { userToken } = useContext(AuthContext);
  const [tab, setTab] = useState<string>(route?.params?.initialTab || 'friends');
  ...
  const sendRequest = async (user: FriendUser): Promise<void> => {
    try {
  ...
  const acceptRequest = async (friendship_id: number | string): Promise<void> => {
    try {
  ...
  const rejectRequest = async (friendship_id: number | string): Promise<void> => {
    try {
  ...
  const removeFriendConfirm = (friendship_id: number | string, name?: string): void => {
  ...
  const viewConquests = (friendId: number | string, friendName?: string): void => {
    navigation.navigate('Conquests', { friendId: String(friendId), friendName });
  };

  const renderSearchResult = ({ item }: { item: FriendUser }) => {
    ...
    let actionBtn: React.ReactElement;
    if (isFriend) {
      actionBtn = (
    ...
    >
      ...
      <View style={styles.cardInfo}>
        ...
        <View style={styles.row}>
          <Text style={{ fontSize: 13 }}>{getRankEmoji(item.conquest_count || 0)}</Text>
          <Text style={styles.rankText}> {getRankTitle(item.conquest_count || 0)}</Text>
  ...
  const renderRequest = ({ item }: { item: FriendRequest }) => (
    <View style={styles.card}>
      <Avatar uri={(item as any).avatar} name={item.name} />
      <View style={styles.cardInfo}>
        {item.username ? <Text style={styles.username}>@{item.username}</Text> : null}
        <Text style={styles.cardName} numberOfLines={1}>{item.name}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {isSearchActive ? (
        searching ? (
          <View style={styles.center}><ActivityIndicator color={THEME.gold} /></View>
...
const styles = StyleSheet.create({
  ...
  actionBtn: { paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
